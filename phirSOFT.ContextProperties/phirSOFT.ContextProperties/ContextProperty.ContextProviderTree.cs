...
namespace phirSOFT.ContextProperties
{
    public partial class ContextProperty<TValue>
    {
        ...
        {
            private readonly Dictionary<IContextProvider<TValue, TValue>, ContextProviderTreeNode> _nodes = new Dictionary<IContextProvider<TValue, TValue>, ContextProviderTreeNode>();
            ...
            IEnumerator<KeyValuePair<IContextProvider<TValue, TValue>, IEnumerable<IContextProvider<TValue, TValue>>>> IEnumerable<KeyValuePair<IContextProvider<TValue, TValue>, IEnumerable<IContextProvider<TValue, TValue>>>>.GetEnumerator()
            {
                return _nodes.Select(kv => new KeyValuePair<IContextProvider<TValue, TValue>, IEnumerable<IContextProvider<TValue, TValue>>>(kv.Key, kv.Value)).GetEnumerator();
            }
            ...
            public ITreeNode<IContextProvider<TValue, TValue>> Root { get; private set; } 
            public bool ContainsKey(IContextProvider<TValue, TValue> key)
            {
                return _nodes.ContainsKey(key);
            }

            public bool TryGetValue(IContextProvider<TValue, TValue> key, out IEnumerable<IContextProvider<TValue, TValue>> value)
            {
            ...
            public IEnumerable<IContextProvider<TValue, TValue>> this[IContextProvider<TValue, TValue> key] => _nodes[key];
